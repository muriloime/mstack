...
description: |
  ...
  what it could look like. (mstack)
triggers:
...
gbrain:
  ...
  context_queries:
    - id: prior-approved-variants
      kind: filesystem
      glob: "~/.mstack/projects/{repo_slug}/designs/*/approved.json"
      sort: mtime_desc
      limit: 5
...
```bash
~/.claude/skills/mstack/bin/mstack-question-log '{"skill":"design-shotgun","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
_PREV=$(find ~/.mstack/projects/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -5)
...
```bash
setopt +o nomatch 2>/dev/null || true
ls ~/.mstack/projects/$SLUG/*office-hours* 2>/dev/null | head -5
```
...
**Persistent taste profile (v1 schema at `~/.mstack/projects/$SLUG/taste-profile.json`):**

Read the persistent taste profile if it exists:

```bash
_TASTE_PROFILE=~/.mstack/projects/$SLUG/taste-profile.json
...
the legacy approved.json aggregate — `~/.claude/skills/mstack/bin/mstack-taste-update`
...
```bash
setopt +o nomatch 2>/dev/null || true
_TASTE=$(find ~/.mstack/projects/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -10)
```
...
variant, call `~/.claude/skills/mstack/bin/mstack-taste-update approved <variant-path>`. When they
explicitly reject a variant, call `~/.claude/skills/mstack/bin/mstack-taste-update rejected <variant-path>`.
...
**Why /tmp/ then cp?** In observed sessions, `$D generate --output ~/.mstack/...`
...
1. **Never save to `.context/`, `docs/designs/`, or `/tmp/`.** All design artifacts go
   to `~/.mstack/projects/$SLUG/designs/`. This is enforced. See DESIGN_SETUP above.
