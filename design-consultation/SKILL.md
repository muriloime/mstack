...
description: |
  ...
  design system or DESIGN.md. (mstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
```bash
~/.claude/skills/mstack/bin/mstack-question-log '{"skill":"design-consultation","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
**Find the mstack designer (optional — enables AI mockup generation):**
...
This phase generates visual previews of the proposed design system. Two paths depending on whether the mstack designer is available.
...
```bash
eval "$(~/.claude/skills/mstack/bin/mstack-slug 2>/dev/null)"
_DESIGN_DIR="$HOME/.mstack/projects/$SLUG/designs/design-system-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
