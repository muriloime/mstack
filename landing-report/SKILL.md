...
description: |
  ...
  the queue", "show me open PRs", or "which version do I claim next". (mstack)
triggers:
  - landing report
  - version queue
  - ship queue
...
```bash
~/.claude/skills/mstack/bin/mstack-question-log '{"skill":"landing-report","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
`bin/mstack-next-version` utility `/ship` uses, but with nothing mutating.
...
Same detection as other mstack skills.
...
for LEVEL in micro patch minor major; do
  bun run bin/mstack-next-version \
    --base "$BASE_BRANCH" \
    --bump "$LEVEL" \
