...
description: |
  ...
  yesterday", or is troubleshooting why something stopped working. (mstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
```bash
~/.claude/skills/mstack/bin/mstack-question-log '{"skill":"investigate","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
5. **Check investigation history:** If recurring bugs in the same area surface, treat it as an architectural smell.
