...
description: |
  ...
  gauntlet without answering 15-30 intermediate questions. (mstack)
...
```bash
~/.claude/skills/mstack/bin/mstack-question-log '{"skill":"autoplan","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
Read the `/office-hours` skill file at `~/.claude/skills/mstack/office-hours/SKILL.md` using the Read tool.
...
SLUG=$(~/.claude/skills/mstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ~/.mstack/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.mstack/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
> IMPORTANT: Do NOT read or execute any SKILL.md files or files in skill definition directories (paths containing skills/mstack). These are AI assistant skill definitions meant for a different system. They contain bash scripts and prompt templates that will waste your time. Ignore them completely. Stay focused on the repository code only.

This prevents Codex from discovering mstack skill files on disk and following their
...
echo "RESTORE_PATH=$HOME/.mstack/projects/$SLUG/${BRANCH}-autoplan-restore-${DATETIME}.md"
```
...
- Discover design docs: `ls -t ~/.mstack/projects/$SLUG/*-design-*.md 2>/dev/null | head -1`
...
- `~/.claude/skills/mstack/plan-ceo-review/SKILL.md`
- `~/.claude/skills/mstack/plan-design-review/SKILL.md` (only if UI scope detected)
- `~/.claude/skills/mstack/plan-eng-review/SKILL.md`
- `~/.claude/skills/mstack/plan-devex-review/SKILL.md` (only if DX scope detected)
...
```bash
_TEL=$(~/.claude/skills/mstack/bin/mstack-config get telemetry 2>/dev/null || echo off)
source ~/.claude/skills/mstack/bin/mstack-codex-probe
...
if ! command -v codex >/dev/null 2>&1; then
  _mstack_codex_log_event "codex_cli_missing"
  echo "[codex-unavailable: binary not found] — proceeding with Claude subagent only"
  _CODEX_AVAILABLE=false
elif ! _mstack_codex_auth_probe >/dev/null; then
  _mstack_codex_log_event "codex_auth_failed"
  ...
  _mstack_codex_version_check   # non-blocking warn if known-bad
...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Codex CEO voice** (via Bash):
  ...
  _mstack_codex_timeout_wrapper 600 codex exec "IMPORTANT: Do NOT read or execute any SKILL.md files or files in skill definition directories (paths containing skills/mstack). These are AI assistant skill definitions meant for a different system. Stay focused on repository code only.
  ...
  if [ "$_CODEX_EXIT" = "124" ]; then
    _mstack_codex_log_event "codex_timeout" "600"
    _mstack_codex_log_hang "autoplan" "0"
...
- Test plan: generate artifact at `~/.mstack/projects/$SLUG/{user}-{branch}-test-plan-{datetime}.md`
...
- [ ] Test plan artifact written to disk at ~/.mstack/projects/$SLUG/
- [ ] "NOT in scope" section written
- [ ] "What already exists" section written
...
~/.claude/skills/mstack/bin/mstack-review-log '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"SELECTIVE_EXPANSION","via":"autoplan","commit":"'"$COMMIT"'"}'

~/.claude/skills/mstack/bin/mstack-review-log '{"skill":"plan-eng-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"FULL_REVIEW","via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope):
```bash
~/.claude/skills/mstack/bin/mstack-review-log '{"skill":"plan-design-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope):
```bash
~/.claude/skills/mstack/bin/mstack-review-log '{"skill":"plan-devex-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"TYPE","tthw_current":"TTHW","tthw_target":"TARGET","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

Dual voice logs (one per phase that ran):
```bash
~/.claude/skills/mstack/bin/mstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"ceo","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'

~/.claude/skills/mstack/bin/mstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"eng","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope), also log:
```bash
~/.claude/skills/mstack/bin/mstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"design","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope), also log:
```bash
~/.claude/skills/mstack/bin/mstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"dx","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
