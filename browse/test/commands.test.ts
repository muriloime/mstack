...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/mstack-cli-');
    const execPath = path.join(root, '.claude/skills/mstack/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/mstack/browse/src/server.ts');
