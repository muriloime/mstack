...
describe('config', () => {
  ...
  describe('resolveConfig', () => {
    test('uses git root by default', () => {
      ...
      expect(config.stateDir).toBe(path.join(gitRoot!, '.mstack'));
      expect(config.stateFile).toBe(path.join(gitRoot!, '.mstack', 'browse.json'));
    });

    test('derives paths from BROWSE_STATE_FILE when set', () => {
      const stateFile = '/tmp/test-config/.mstack/browse.json';
      const config = resolveConfig({ BROWSE_STATE_FILE: stateFile });
      expect(config.stateFile).toBe(stateFile);
      expect(config.stateDir).toBe('/tmp/test-config/.mstack');
  ...
  describe('ensureStateDir', () => {
    test('creates directory if it does not exist', () => {
      ...
      const config = resolveConfig({ BROWSE_STATE_FILE: path.join(tmpDir, '.mstack', 'browse.json') });
    ...
    test('is a no-op if directory already exists', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-config-test-${Date.now()}`);
      const stateDir = path.join(tmpDir, '.mstack');
    ...
    test('adds .mstack/ to .gitignore if not present', () => {
      ...
      expect(content).toContain('.mstack/');
      expect(content).toBe('node_modules/\n.mstack/\n');
      fs.rmSync(tmpDir, { recursive: true, force: true });
    });

    test('does not duplicate .mstack/ in .gitignore', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-gitignore-test-${Date.now()}`);
      fs.mkdirSync(tmpDir, { recursive: true });
      fs.writeFileSync(path.join(tmpDir, '.gitignore'), 'node_modules/\n.mstack/\n');
      const config = resolveConfig({ BROWSE_STATE_FILE: path.join(tmpDir, '.mstack', 'browse.json') });
    ...
    test('logs warning to browse-server.log on non-ENOENT gitignore error', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-gitignore-test-${Date.now()}`);
      fs.mkdirSync(tmpDir, { recursive: true });
      // Create a read-only .gitignore (no .mstack/ entry → would try to append)
      fs.writeFileSync(path.join(tmpDir, '.gitignore'), 'node_modules/\n');
...
describe('resolveGstackHome', () => {
  test('honors MSTACK_HOME env var when set', () => {
    const orig = process.env.MSTACK_HOME;
    process.env.MSTACK_HOME = '/tmp/custom-mstack-home';
    try {
      expect(resolveGstackHome()).toBe('/tmp/custom-mstack-home');
    } finally {
      if (orig === undefined) delete process.env.MSTACK_HOME;
      else process.env.MSTACK_HOME = orig;
    }
  });

  test('falls back to os.homedir() + /.mstack when env unset', () => {
    const orig = process.env.MSTACK_HOME;
    delete process.env.MSTACK_HOME;
    try {
      expect(resolveGstackHome()).toBe(path.join(os.homedir(), '.mstack'));
    } finally {
      if (orig !== undefined) process.env.MSTACK_HOME = orig;
    }
  });
});
...
describe('resolveChromiumProfile', () => {
  ...
  test('falls back to resolveGstackHome()/chromium-profile when nothing set', () => {
    ...
    const origHome = process.env.MSTACK_HOME;
    delete process.env.CHROMIUM_PROFILE;
    process.env.MSTACK_HOME = '/tmp/fallback-mstack';
    try {
      expect(resolveChromiumProfile()).toBe('/tmp/fallback-mstack/chromium-profile');
    } finally {
      if (origEnv !== undefined) process.env.CHROMIUM_PROFILE = origEnv;
      if (origHome === undefined) delete process.env.MSTACK_HOME;
      else process.env.MSTACK_HOME = origHome;
    }
  });
