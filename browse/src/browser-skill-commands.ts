 ...
 * and passes it via MSTACK_SKILL_TOKEN. The skill never sees the daemon root
 ...
 *    MSTACK_PORT and MSTACK_SKILL_TOKEN are always set.
 ...
 * Includes: minimal PATH, locale, terminal type. Skills get MSTACK_PORT +
 * MSTACK_SKILL_TOKEN injected separately.
 */
...
export function buildSpawnEnv(opts: BuildEnvOptions): Record<string, string> {
  ...
  if (opts.trusted) {
    ...
    for (const [k, v] of Object.entries(process.env)) {
      ...
      if (k === 'MSTACK_TOKEN') continue; // never propagate root token
      out[k] = v;
    }
  ...
  // intentionally — e.g. an internal-tool skill — but we still strip MSTACK_TOKEN
  // above.)
  if (!opts.trusted) {
  ...
  out.MSTACK_PORT = String(opts.port);
  out.MSTACK_SKILL_TOKEN = opts.skillToken;
