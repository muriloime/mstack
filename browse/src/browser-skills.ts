 ...
 *   project:  <project>/.mstack/browser-skills/<name>/
 *   global:   ~/.mstack/browser-skills/<name>/
 *   bundled:  <mstack-install>/browser-skills/<name>/   (read-only, ships with mstack)
 *
...
export function defaultTierPaths(opts: { projectRoot?: string; home?: string; bundledRoot?: string } = {}): TierPaths {
  ...
  return {
    project: projectRoot ? path.join(projectRoot, '.mstack', 'browser-skills') : null,
    global: path.join(home, '.mstack', 'browser-skills'),
...
function detectBundledRoot(): string {
  // The browse binary lives at <mstack-install>/browse/dist/browse.
  // The bundled browser-skills/ dir is a sibling of browse/ (i.e. <mstack-install>/browser-skills/).
  ...
  // browse/src/browser-skills.ts → ../../  (the mstack root).
 ...
 *
 * Cannot tombstone bundled skills — they ship with mstack and are read-only.
 * To remove a bundled skill, override it with a global/project entry, or
 * remove the file from the mstack source tree.
 */
