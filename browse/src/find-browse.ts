/**
 * find-browse — locate the mstack browse binary.
 *
...
export function locateBinary(): string | null {
  ...
  if (root) {
    for (const m of markers) {
      const local = join(root, m, 'skills', 'mstack', 'browse', 'dist', 'browse');
  ...
  // Global fallback
  for (const m of markers) {
    const global = join(home, m, 'skills', 'mstack', 'browse', 'dist', 'browse');
