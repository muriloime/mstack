 ...
 *
 * To upgrade: bump this string, run `MSTACK_BENCH_ENSEMBLE=1 bun test
 ...
 * layout at ~/.mstack/models/testsavant-small/ on first use.
 *
 ...
 */
const MODELS_DIR = path.join(os.homedir(), '.mstack', 'models');
...
// MSTACK_SECURITY_ENSEMBLE=deberta. Not forced on every install because
...
function isDebertaEnabled(): boolean {
  const setting = (process.env.MSTACK_SECURITY_ENSEMBLE ?? '').toLowerCase();
...
export function loadTestsavant(onProgress?: (msg: string) => void): Promise<void> {
  if (process.env.MSTACK_SECURITY_OFF === '1') {
    testsavantState = 'failed';
    testsavantLoadError = 'MSTACK_SECURITY_OFF=1 — ML classifier kill switch engaged';
...
export function loadDeberta(onProgress?: (msg: string) => void): Promise<void> {
  if (process.env.MSTACK_SECURITY_OFF === '1') return Promise.resolve();
...
export async function checkTranscript(params: {
  ...
  return new Promise((resolve) => {
    ...
    // repo with a prompt-injection-defense CLAUDE.md (like mstack itself),
    ...
    // Env var MSTACK_HAIKU_TIMEOUT_MS (milliseconds) overrides for benches
    // that want a different budget.
    const timeoutMs = process.env.MSTACK_HAIKU_TIMEOUT_MS
      ? Number(process.env.MSTACK_HAIKU_TIMEOUT_MS)
      : 45000;
    setTimeout(() => {
