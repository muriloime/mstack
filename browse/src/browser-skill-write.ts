 ...
 *
 * /skillify stages a candidate skill into ~/.mstack/.tmp/skillify-<spawnId>/,
 ...
 *   <tmpRoot>/.mstack/.tmp/skillify-<spawnId>/<name>/
 *
...
export function stageSkill(opts: StageSkillOptions): string {
  ...
  const tmpRoot = opts.tmpRoot ?? path.join(os.homedir(), '.mstack', '.tmp');
