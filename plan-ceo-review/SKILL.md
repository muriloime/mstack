...
description: |
  ...
  or when the plan feels like it could be thinking bigger. (mstack)
...
gbrain:
  ...
  context_queries:
    - id: prior-ceo-plans
      kind: filesystem
      glob: "~/.mstack/projects/{repo_slug}/ceo-plans/*.md"
      sort: mtime_desc
      limit: 5
...
```bash
~/.claude/skills/mstack/bin/mstack-question-log '{"skill":"plan-ceo-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
HANDOFF=$(ls -t ~/.mstack/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
```bash
eval "$(~/.claude/skills/mstack/bin/mstack-slug 2>/dev/null)" && mkdir -p ~/.mstack/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.mstack/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.mstack/projects/$SLUG/ceo-plans/{old-plan}.md ~/.mstack/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.mstack/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.mstack/analytics/spec-review.jsonl 2>/dev/null || true
```
...
```
NOTE: These represent human-team implementation hours. With CC + mstack,
...
**Persist the result:**
```bash
~/.claude/skills/mstack/bin/mstack-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
* **Effort estimate:** S/M/L/XL (human team) → with CC+mstack: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
eval "$(~/.claude/skills/mstack/bin/mstack-slug 2>/dev/null)"
rm -f ~/.mstack/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null || true
```
...
```bash
~/.claude/skills/mstack/bin/mstack-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","scope_proposed":N,"scope_accepted":N,"scope_deferred":N,"commit":"COMMIT"}'
```
...
- **B)** Keep in `~/.mstack/projects/` only (local, personal reference)
- **C)** Skip
