...
description: |
  ...
  or "post-ship docs". Proactively suggest after a PR is merged or code is shipped. (mstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
```bash
~/.claude/skills/mstack/bin/mstack-question-log '{"skill":"document-release","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
find . -maxdepth 2 -name "*.md" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.mstack/*" -not -path "./.context/*" | sort
```
...
(adapt to whatever project you're in — these are not mstack-specific):
...
**If GitHub:**
```bash
gh pr view --json body -q .body > /tmp/mstack-pr-body-$$.md
```

**If GitLab:**
```bash
glab mr view -F json 2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin).get('description',''))" > /tmp/mstack-pr-body-$$.md
```
...
**If GitHub:**
```bash
gh pr edit --body-file /tmp/mstack-pr-body-$$.md
```

**If GitLab:**
Read the contents of `/tmp/mstack-pr-body-$$.md` using the Read tool, then pass it to `glab mr update` using a heredoc to avoid shell metacharacter issues:
```bash
...
```bash
NEW_TITLE=$(~/.claude/skills/mstack/bin/mstack-pr-title-rewrite.sh "$V" "$CURRENT_TITLE")
```
