...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/mstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
PROJECT_HISTORY="$HOME/.mstack/projects/$REMOTE_SLUG/greptile-history.md"
```
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/mstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/.mstack/projects/$REMOTE_SLUG"
mkdir -p ~/.mstack
```

Append one line per triage outcome to **both** files (per-project for suppressions, global for retro):
- `~/.mstack/projects/$REMOTE_SLUG/greptile-history.md` (per-project)
- `~/.mstack/greptile-history.md` (global aggregate)
