...
Thanks for wanting to make mstack better. Whether you're fixing a typo in a skill prompt or building an entirely new workflow, this guide will get you up and running fast.

## Quick start

mstack skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `~/.claude/skills/mstack/` (your global install). But when you're developing mstack itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.

That's what dev mode does. It symlinks your repo into the local `.claude/skills/` directory so Claude Code reads skills straight from your checkout.

```bash
git clone https://github.com/garrytan/gstack.git && cd mstack
...
mstack automatically learns from failures. At the end of every skill session, the agent
reflects on what went wrong (CLI errors, wrong approaches, project quirks) and logs
operational learnings to `~/.mstack/projects/{slug}/learnings.jsonl`. Future sessions
surface these learnings automatically, so mstack gets smarter on your codebase over time.

No setup needed. Learnings are logged automatically. View them with `/learn`.

### The contributor workflow

1. **Use mstack normally** — operational learnings are captured automatically
2. **Check your learnings:** `/learn` or `ls ~/.mstack/projects/*/learnings.jsonl`
3. **Fork and clone mstack** (if you haven't already)
4. **Symlink your fork into the project where you hit the bug:**
   ```bash
   # In your core project (the one where mstack annoyed you)
   ln -sfn /path/to/your/mstack-fork .claude/skills/mstack
   cd .claude/skills/mstack && bun install && bun run build && ./setup
   ```
   Setup creates per-skill directories with SKILL.md symlinks inside (`qa/SKILL.md -> mstack/qa/SKILL.md`)
   and asks your prefix preference. Pass `--no-prefix` to skip the prompt and use short names.
5. **Fix the issue** — your changes are live immediately in this project
6. **Test by actually using mstack** — do the thing that annoyed you, verify it's fixed
7. **Open a PR from your fork**

This is the best way to contribute: fix mstack while doing your real work, in the
project where you actually felt the pain.

### Session awareness

When you have 3+ mstack sessions open simultaneously, every question tells you which project, which branch, and what's happening. No more staring at a question thinking "wait, which window is this?" The format is consistent across all skills.

## Working on mstack inside the mstack repo

When you're editing mstack skills and want to test them by actually using mstack
...
```
mstack/                          <- your working tree
├── .claude/skills/              <- created by dev-setup (gitignored)
│   ├── mstack -> ../../         <- symlink back to repo root
│   ├── review/                  <- real directory (short name, default)
│   │   └── SKILL.md -> mstack/review/SKILL.md
│   ├── ship/                    <- or mstack-review/, mstack-ship/ if --prefix
│   │   └── SKILL.md -> mstack/ship/SKILL.md
...
under `mstack/`. Names depend on your prefix setting (`~/.mstack/config.yaml`).
Short names (`/review`, `/ship`) are the default. Run `./setup --prefix` if you
prefer namespaced names (`/mstack-review`, `/mstack-ship`).
...
When E2E tests run, they produce machine-readable artifacts in `~/.mstack-dev/`:
...
Artifacts are never cleaned up — they accumulate in `~/.mstack-dev/` for post-mortem debugging and trend analysis.
...
mstack's Writing Style section (injected into every tier-≥2 skill's preamble)
...
mstack generates SKILL.md files for 8 hosts from one set of `.tmpl` templates.
...
|--------|---------------------------|
| Output directory | `{skill}/SKILL.md` vs `.agents/skills/mstack-{skill}/SKILL.md` |
| Frontmatter | Full (name, description, hooks, version) vs minimal (name + description) |
| Paths | `~/.claude/skills/mstack` vs `$MSTACK_ROOT` |
...
**`MSTACK_*` env prefix (Conductor-injected keys).** Conductor explicitly strips `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` from every workspace's process env. The `.env` copy path doesn't restore them either — the strip happens after env inheritance. Users who want paid evals, `/sync-gbrain` embeddings, or `claude-agent-sdk` calls to work in a Conductor workspace must set `MSTACK_ANTHROPIC_API_KEY` and `MSTACK_OPENAI_API_KEY` in Conductor's workspace env config; Conductor passes those through untouched. On the mstack side, TS entry points import `lib/conductor-env-shim.ts` as a side effect, which promotes `MSTACK_FOO_API_KEY` to `FOO_API_KEY` when the canonical name is empty. If you add a new TS entry point that hits a paid API, add `import "../lib/conductor-env-shim";` to the top of the file. Today the shim is imported from `bin/mstack-gbrain-sync.ts`, `bin/mstack-model-benchmark`, `scripts/preflight-agent-sdk.ts`, and `test/helpers/e2e-helpers.ts`.
...
- **Dev mode shadows your global install.** Project-local skills take priority over `~/.claude/skills/mstack`. `bin/dev-teardown` restores the global one.
...
**This is the recommended way to develop mstack.** Symlink your mstack checkout
...
```bash
# In your core project (not the mstack repo)
ln -sfn /path/to/your/mstack-checkout .claude/skills/mstack
```

### Step 2: Run setup to create per-skill symlinks

The `mstack` symlink alone isn't enough. Claude Code discovers skills through
individual top-level directories (`qa/SKILL.md`, `ship/SKILL.md`, etc.), not through
the `mstack/` directory itself. Run `./setup` to create them:

```bash
cd .claude/skills/mstack && bun install && bun run build && ./setup
```

Setup will ask whether you want short names (`/qa`) or namespaced (`/mstack-qa`).
Your choice is saved to `~/.mstack/config.yaml` and remembered for future runs.
...
Remove the project-local symlink. Claude Code falls back to `~/.claude/skills/mstack/`:

```bash
rm .claude/skills/mstack
```

The per-skill directories (`qa/`, `ship/`, etc.) contain SKILL.md symlinks that point
to `mstack/...`, so they'll resolve to the global install automatically.

### Switching prefix mode

If you installed mstack with one prefix setting and want to switch:

```bash
cd .claude/skills/mstack && ./setup --no-prefix   # switch to /qa, /ship
cd .claude/skills/mstack && ./setup --prefix       # switch to /mstack-qa, /mstack-ship
```
...
- Renamed or moved config keys in `~/.mstack/config.yaml`
- Need to delete orphaned files from a previous version
- Changed the format of `~/.mstack/` state files
...
1. Create `mstack-upgrade/migrations/v{VERSION}.sh` where `{VERSION}` matches
   the VERSION file for the release that needs the fix.
2. Make it executable: `chmod +x mstack-upgrade/migrations/v{VERSION}.sh`
...
"$SCRIPT_DIR/bin/mstack-relink" 2>/dev/null || true
```

### How it runs

During `/mstack-upgrade`, after `./setup` completes (Step 4.75), the upgrade
skill scans `mstack-upgrade/migrations/` and runs every `v*.sh` script whose
...
verifies that all migration scripts in `mstack-upgrade/migrations/` are
