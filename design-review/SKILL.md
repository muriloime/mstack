...
description: |
  ...
  wants to polish the look of a live site. (mstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
```bash
~/.claude/skills/mstack/bin/mstack-question-log '{"skill":"design-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
**Find the mstack designer (optional — enables target mockup generation):**
...
```bash
eval "$(~/.claude/skills/mstack/bin/mstack-slug 2>/dev/null)"
REPORT_DIR="$HOME/.mstack/projects/$SLUG/designs/design-audit-$(date +%Y%m%d)"
...
**Local:** `.mstack/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:**
```bash
eval "$(~/.claude/skills/mstack/bin/mstack-slug 2>/dev/null)" && mkdir -p ~/.mstack/projects/$SLUG
```
Write to: `~/.mstack/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md`
...
```
~/.mstack/projects/$SLUG/designs/design-audit-{YYYYMMDD}/
...
If the mstack designer is available and the finding involves visual layout, hierarchy, or spacing (not just a CSS value fix like wrong color or font-size), generate a target mockup showing what the corrected version should look like:
...
eval "$(~/.claude/skills/mstack/bin/mstack-slug 2>/dev/null)" && mkdir -p ~/.mstack/projects/$SLUG
```
Write a one-line summary to `~/.mstack/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
