 ...
 * Conductor's MSTACK_-prefixed forms are present. Promote the MSTACK_ form to
...
export function promoteConductorEnv(): void {
  for (const key of ["ANTHROPIC_API_KEY", "OPENAI_API_KEY"] as const) {
    if (!process.env[key] && process.env[`MSTACK_${key}`]) {
      process.env[key] = process.env[`MSTACK_${key}`];
    }
  }
}
