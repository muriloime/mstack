...
const MSTACK_DEV_DIR = path.join(os.homedir(), '.mstack-dev');
const HEARTBEAT_PATH = path.join(MSTACK_DEV_DIR, 'e2e-live.json'); // heartbeat stays global
const PROJECT_DIR = path.dirname(getProjectEvalDir()); // ~/.mstack/projects/$SLUG/
...
export async function runSkillTest(options: {
   ...
   *  per-test MSTACK_HOME overrides so the test doesn't have to spell out
  ...
  if (browseErrors.length > 0 || exitReason !== 'success') {
    try {
      const failureDir = runDir || path.join(workingDirectory, '.mstack', 'test-transcripts');
