 ...
 *   - telemetry JSONL under $HOME/.mstack/analytics/ (same walk, but called
...
export interface SinkResult {
  ...
  /** Subset of filesWritten matching .mstack/analytics/*.jsonl. */
...
export async function runWithSecretSink(opts: SecretSinkOptions): Promise<SinkResult> {
  ...
  // Make sure .mstack exists so bins that append to analytics have somewhere to write.
  fs.mkdirSync(path.join(tmpHome, '.mstack', 'analytics'), { recursive: true });

  const env = {
    // Minimal PATH that still finds jq/git/curl/sed so our bins work.
    PATH: '/usr/bin:/bin:/usr/sbin:/sbin:/opt/homebrew/bin:/usr/local/bin',
    HOME: tmpHome,
    MSTACK_HOME: path.join(tmpHome, '.mstack'),
    ...(opts.env || {}),
  };
  ...
  for (const [rel, content] of Object.entries(filesWritten)) {
    if (rel.startsWith('.mstack/analytics/') && rel.endsWith('.jsonl')) {
  ...
  for (const seed of opts.seeds) {
    ...
    for (const { rule, matchType } of rules) {
      ...
      for (const [rel, content] of Object.entries(filesWritten)) {
        ...
        if (hit !== null) {
          const channel = rel.startsWith('.mstack/analytics/') ? 'telemetry' : 'file';
