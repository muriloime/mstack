...
describeIfSelected('PlanTune E2E', ['plan-tune-inspect'], () => {
  ...
  beforeAll(() => {
    ...
    // mstack-developer-profile --derive imports from scripts/ — copy those too.
    ...
    const cfgDir = path.join(mstackHome);
...
IMPORTANT:
- Use MSTACK_HOME="${mstackHome}" as an environment variable for all bin calls.
- Replace any ~/.claude/skills/mstack/bin/ references with ./bin/ (relative path).
- Replace any ~/.claude/skills/mstack/scripts/ references with ./scripts/.
