...
// Eval result collector — accumulates test results, writes to ~/.mstack-dev/evals/ on finalize
...
function dumpOutcomeDiagnostic(dir: string, label: string, report: string, judgeResult: any) {
  try {
    const transcriptDir = path.join(dir, '.mstack', 'test-transcripts');
...
Report whether it worked.`,
  ...
  testIfSelected('skillmd-no-local-binary', async () => {
    // Create a tmpdir with no browse binary — no local .claude/skills/mstack/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // On dev machines with mstack installed globally, the fallback path
    // ~/.claude/skills/mstack/browse/dist/browse exists, so we get READY.
...
Report the exact output — either "READY: <path>" or "NEEDS_SETUP".`,
  ...
  testIfSelected('session-awareness', async () => {
    ...
    const result = await runSkillTest({
      prompt: `You are running a mstack skill. The session preamble detected _SESSIONS=4 (the user has 4 mstack windows open).
...
Write your report to ${qaOnlyDir}/qa-reports/qa-only-report.md`,
    ...
    const statusLines = gitStatus.stdout.toString().trim().split('\n').filter(
      (l: string) => l.trim() && !l.includes('.prompt-tmp') && !l.includes('.mstack/') && !l.includes('qa-reports/'),
    );
...
`);
    ...
    projectDir = path.join(os.homedir(), '.mstack', 'projects', 'test-project');
  ...
  test('/plan-eng-review writes test-plan artifact to ~/.mstack/projects/', async () => {
...
describeIfSelected('mstack-upgrade E2E', ['mstack-upgrade-happy-path'], () => {
  let upgradeDir: string;
  let remoteDir: string;

  beforeAll(() => {
    upgradeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'skill-e2e-upgrade-'));
    remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'mstack-remote-'));
    ...
    // Create mock mstack install directory (local-git type)
    const mockGstack = path.join(upgradeDir, '.claude', 'skills', 'mstack');
    ...
    // Copy mstack-upgrade skill
    fs.mkdirSync(path.join(upgradeDir, 'mstack-upgrade'), { recursive: true });
    fs.copyFileSync(
      path.join(ROOT, 'mstack-upgrade', 'SKILL.md'),
      path.join(upgradeDir, 'mstack-upgrade', 'SKILL.md'),
    );
  ...
  testIfSelected('mstack-upgrade-happy-path', async () => {
    const mockGstack = path.join(upgradeDir, '.claude', 'skills', 'mstack');
    const result = await runSkillTest({
      prompt: `Read mstack-upgrade/SKILL.md for the upgrade workflow.

You are running /mstack-upgrade standalone. The mstack installation is at ./.claude/skills/mstack (local-git type — it has a .git directory with an origin remote).
...
IMPORTANT: The install directory is at ./.claude/skills/mstack — use that exact path.`,
      workingDirectory: upgradeDir,
      maxTurns: 20,
      timeout: 180_000,
      testName: 'mstack-upgrade-happy-path',
      runId,
    });

    logCost('/mstack-upgrade happy path', result);
    ...
    recordE2E('/mstack-upgrade happy path', 'mstack-upgrade E2E', result, {
