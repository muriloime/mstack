...
description: |
  ...
  is ready, asks about deploying, wants to push code up, or asks to create a PR. (mstack)
user-invocable: true
...
MSTACK_ROOT="$HOME/.factory/skills/mstack"
[ -n "$_ROOT" ] && [ -d "$_ROOT/.factory/skills/mstack" ] && MSTACK_ROOT="$_ROOT/.factory/skills/mstack"
MSTACK_BIN="$MSTACK_ROOT/bin"
MSTACK_BROWSE="$MSTACK_ROOT/browse/dist"
MSTACK_DESIGN="$MSTACK_ROOT/design/dist"
_UPD=$($MSTACK_BIN/mstack-update-check 2>/dev/null || .factory/skills/mstack/bin/mstack-update-check 2>/dev/null || true)
...
_PROACTIVE=$($MSTACK_BIN/mstack-config get proactive 2>/dev/null || echo "true")
_PROACTIVE_PROMPTED=$([ -f ~/.mstack/.proactive-prompted ] && echo "yes" || echo "no")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
_SKILL_PREFIX=$($MSTACK_BIN/mstack-config get skill_prefix 2>/dev/null || echo "false")
echo "PROACTIVE: $_PROACTIVE"
echo "PROACTIVE_PROMPTED: $_PROACTIVE_PROMPTED"
echo "SKILL_PREFIX: $_SKILL_PREFIX"
source <($MSTACK_BIN/mstack-repo-mode 2>/dev/null) || true
REPO_MODE=${REPO_MODE:-unknown}
echo "REPO_MODE: $REPO_MODE"
_LAKE_SEEN=$([ -f ~/.mstack/.completeness-intro-seen ] && echo "yes" || echo "no")
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$($MSTACK_BIN/mstack-config get telemetry 2>/dev/null || true)
_TEL_PROMPTED=$([ -f ~/.mstack/.telemetry-prompted ] && echo "yes" || echo "no")
_TEL_START=$(date +%s)
_SESSION_ID="$$-$(date +%s)"
echo "TELEMETRY: ${_TEL:-off}"
echo "TEL_PROMPTED: $_TEL_PROMPTED"
_EXPLAIN_LEVEL=$($MSTACK_BIN/mstack-config get explain_level 2>/dev/null || echo "default")
if [ "$_EXPLAIN_LEVEL" != "default" ] && [ "$_EXPLAIN_LEVEL" != "terse" ]; then _EXPLAIN_LEVEL="default"; fi
echo "EXPLAIN_LEVEL: $_EXPLAIN_LEVEL"
_QUESTION_TUNING=$($MSTACK_BIN/mstack-config get question_tuning 2>/dev/null || echo "false")
echo "QUESTION_TUNING: $_QUESTION_TUNING"
mkdir -p ~/.mstack/analytics
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ship","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.mstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
for _PF in $(find ~/.mstack/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do
  if [ -f "$_PF" ]; then
    if [ "$_TEL" != "off" ] && [ -x "$MSTACK_BIN/mstack-telemetry-log" ]; then
      $MSTACK_BIN/mstack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true
    fi
    rm -f "$_PF" 2>/dev/null || true
  fi
  break
done
eval "$($MSTACK_BIN/mstack-slug 2>/dev/null)" 2>/dev/null || true
_LEARN_FILE="${MSTACK_HOME:-$HOME/.mstack}/projects/${SLUG:-unknown}/learnings.jsonl"
if [ -f "$_LEARN_FILE" ]; then
  _LEARN_COUNT=$(wc -l < "$_LEARN_FILE" 2>/dev/null | tr -d ' ')
  echo "LEARNINGS: $_LEARN_COUNT entries loaded"
  if [ "$_LEARN_COUNT" -gt 5 ] 2>/dev/null; then
    $MSTACK_BIN/mstack-learnings-search --limit 3 2>/dev/null || true
  fi
else
  echo "LEARNINGS: 0"
fi
$MSTACK_BIN/mstack-timeline-log '{"skill":"ship","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
if [ -f CLAUDE.md ] && grep -q "## Skill routing" CLAUDE.md 2>/dev/null; then
  _HAS_ROUTING="yes"
fi
_ROUTING_DECLINED=$($MSTACK_BIN/mstack-config get routing_declined 2>/dev/null || echo "false")
echo "HAS_ROUTING: $_HAS_ROUTING"
echo "ROUTING_DECLINED: $_ROUTING_DECLINED"
_VENDORED="no"
if [ -d ".factory/skills/mstack" ] && [ ! -L ".factory/skills/mstack" ]; then
  if [ -f ".factory/skills/mstack/VERSION" ] || [ -d ".factory/skills/mstack/.git" ]; then
    _VENDORED="yes"
  fi
fi
echo "VENDORED_MSTACK: $_VENDORED"
echo "MODEL_OVERLAY: claude"
_CHECKPOINT_MODE=$($MSTACK_BIN/mstack-config get checkpoint_mode 2>/dev/null || echo "explicit")
_CHECKPOINT_PUSH=$($MSTACK_BIN/mstack-config get checkpoint_push 2>/dev/null || echo "false")
...
If `SKILL_PREFIX` is `"true"`, suggest/invoke `/mstack-*` names. Disk paths stay `$MSTACK_ROOT/[skill-name]/SKILL.md`.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `$MSTACK_ROOT/mstack-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined).

If output shows `JUST_UPGRADED <from> <to>`: print "Running mstack v{to} (just updated!)". If `SPAWNED_SESSION` is true, skip feature discovery.

Feature discovery, max one prompt per session:
- Missing `$MSTACK_ROOT/.feature-prompted-continuous-checkpoint`: AskUserQuestion for Continuous checkpoint auto-commits. If accepted, run `$MSTACK_BIN/mstack-config set checkpoint_mode continuous`. Always touch marker.
- Missing `$MSTACK_ROOT/.feature-prompted-model-overlay`: inform "Model overlays are active. MODEL_OVERLAY shows the patch." Always touch marker.
...
If B: run `$MSTACK_BIN/mstack-config set explain_level terse`.
...
If A: run `$MSTACK_BIN/mstack-config set telemetry community`
...
If B→A: run `$MSTACK_BIN/mstack-config set telemetry anonymous`
If B→B: run `$MSTACK_BIN/mstack-config set telemetry off`
...
If A: run `$MSTACK_BIN/mstack-config set proactive true`
If B: run `$MSTACK_BIN/mstack-config set proactive false`
...
If B: run `$MSTACK_BIN/mstack-config set routing_declined true` and say they can re-enable with `mstack-config set routing_declined false`.

This only happens once per project. Skip if `HAS_ROUTING` is `yes` or `ROUTING_DECLINED` is `true`.

If `VENDORED_MSTACK` is `yes`, warn once via AskUserQuestion unless `~/.mstack/.vendoring-warned-$SLUG` exists:

> This project has mstack vendored in `.factory/skills/mstack/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r .factory/skills/mstack/`
2. Run `echo '.factory/skills/mstack/' >> .gitignore`
3. Run `$MSTACK_BIN/mstack-team-init required` (or `optional`)
4. Run `git add .claude/ .gitignore CLAUDE.md && git commit -m "chore: migrate mstack from vendored to team mode"`
5. Tell the user: "Done. Each developer now runs: `cd $MSTACK_ROOT && ./setup --team`"
...
_BRAIN_SYNC_BIN="$MSTACK_BIN/gstack-brain-sync"
_BRAIN_CONFIG_BIN="$MSTACK_BIN/mstack-config"
...
```bash
"$MSTACK_BIN/gstack-brain-sync" --discover-new 2>/dev/null || true
"$MSTACK_BIN/gstack-brain-sync" --once 2>/dev/null || true
```
...
```bash
eval "$($MSTACK_BIN/mstack-slug 2>/dev/null)"
...
Before each AskUserQuestion, choose `question_id` from `scripts/question-registry.ts` or `{skill}-{slug}`, then run `$MSTACK_BIN/mstack-question-preference --check "<id>"`. `AUTO_DECIDE` means choose the recommended option and say "Auto-decided [summary] → [option] (your preference). Change with /plan-tune." `ASK_NORMALLY` means ask.

After answer, log best-effort:
```bash
$MSTACK_BIN/mstack-question-log '{"skill":"ship","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
$MSTACK_BIN/mstack-question-preference --write '{"question_id":"<id>","preference":"<pref>","source":"inline-user","free_text":"<optional original words>"}'
```
...
Before building anything unfamiliar, **search first.** See `$MSTACK_ROOT/ETHOS.md`.
...
```bash
$MSTACK_BIN/mstack-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
```
...
`~/.mstack/analytics/`, matching preamble analytics writes.
...
$MSTACK_ROOT/bin/mstack-timeline-log '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
...
if [ "$_TEL" != "off" ] && [ -x $MSTACK_ROOT/bin/mstack-telemetry-log ]; then
  $MSTACK_ROOT/bin/mstack-telemetry-log \
...
```bash
$MSTACK_ROOT/bin/mstack-review-read
```
...
**Review tiers:**
- **Eng Review (required by default):** The only review that gates shipping. Covers architecture, code quality, tests, performance. Can be disabled globally with \`mstack-config set skip_eng_review true\` (the "don't bother me" setting).
...
For Design Review: run `source <($MSTACK_ROOT/bin/mstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
[ -f .mstack/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `.mstack/no-test-bootstrap` and continue without tests.
...
If user picks C → write `.mstack/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.mstack/no-test-bootstrap` and re-run." Continue without tests.
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `.factory/skills/mstack/review/TODOS-format.md`).
...
- Create an issue assigned to that person (use the platform detected in Step 0):
  - **If GitHub:**
    ...
    gh issue create \
      ...
      --body "Found failing on branch <current-branch>. Failure is pre-existing.\n\n**Error:**\n```\n<first 10 lines>\n```\n\n**Last modified by:** <author>\n**Noticed by:** mstack /ship on <date>" \
...
```bash
eval "$($MSTACK_ROOT/bin/mstack-slug 2>/dev/null)" && mkdir -p ~/.mstack/projects/$SLUG
USER=$(whoami)
DATETIME=$(date +%Y%m%d-%H%M%S)
```

Write to `~/.mstack/projects/{slug}/{user}-{branch}-ship-test-plan-{datetime}.md`:
...
# Compute project slug for ~/.mstack/projects/ lookup
_PLAN_SLUG=$(git remote get-url origin 2>/dev/null | sed 's|.*[:/]\([^/]*/[^/]*\)\.git$|\1|;s|.*[:/]\([^/]*/[^/]*\)$|\1|' | tr '/' '-' | tr -cd 'a-zA-Z0-9._-') || true
_PLAN_SLUG="${_PLAN_SLUG:-$(basename "$PWD" | tr -cd 'a-zA-Z0-9._-')}"
# Search common plan file locations (project designs first, then personal/local)
for PLAN_DIR in "$HOME/.mstack/projects/$_PLAN_SLUG" "$HOME/.claude/plans" "$HOME/.codex/plans" ".mstack/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
```bash
_CROSS_PROJ=$($MSTACK_BIN/mstack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  $MSTACK_BIN/mstack-learnings-search --limit 10 --query "release ship version changelog merge pr" --cross-project 2>/dev/null || true
else
  $MSTACK_BIN/mstack-learnings-search --limit 10 --query "release ship version changelog merge pr" 2>/dev/null || true
fi
```
...
If A: run `$MSTACK_BIN/mstack-config set cross_project_learnings true`
If B: run `$MSTACK_BIN/mstack-config set cross_project_learnings false`
...
1. Read `.factory/skills/mstack/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
Check if the diff touches frontend files using `mstack-diff-scope`:

```bash
source <($MSTACK_BIN/mstack-diff-scope <base> 2>/dev/null)
```
...
2. **Read `.factory/skills/mstack/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
```bash
$MSTACK_BIN/mstack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
source <($MSTACK_BIN/mstack-diff-scope <base> 2>/dev/null) || true
...
```bash
$MSTACK_BIN/mstack-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `$MSTACK_ROOT/review/specialists/testing.md`
2. **Maintainability** — read `$MSTACK_ROOT/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `$MSTACK_ROOT/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `$MSTACK_ROOT/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `$MSTACK_ROOT/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `$MSTACK_ROOT/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `$MSTACK_ROOT/review/design-checklist.md`
...
```bash
$MSTACK_BIN/mstack-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `$MSTACK_ROOT/review/specialists/red-team.md`
...
```bash
$MSTACK_ROOT/bin/mstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
> You are classifying Greptile review comments for a /ship workflow. Read `.factory/skills/mstack/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>
...
OLD_CFG=$($MSTACK_ROOT/bin/mstack-config get codex_reviews 2>/dev/null || true)
...
```bash
$MSTACK_ROOT/bin/mstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
$MSTACK_BIN/mstack-learnings-log '{"skill":"ship","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
```bash
$MSTACK_ROOT/bin/mstack-learnings-search --query "<your-keyword>" --limit 5 2>/dev/null || true
```
...
3. **Queue-aware version pick (workspace-aware ship, v1.6.4.0+).** Call `bin/mstack-next-version` to see what's already claimed by open PRs + active sibling Conductor worktrees, then render the queue state to the user:

   ```bash
   QUEUE_JSON=$(bun run bin/mstack-next-version \
     --base <base> \
     --bump "$BUMP_LEVEL" \
...
Read `.factory/skills/mstack/review/TODOS-format.md` for the canonical format reference.
...
```bash
# Export [mstack-context] blocks from all WIP commits on this branch.
# This file becomes input to the CHANGELOG entry and may inform PR body context.
mkdir -p "$(git rev-parse --show-toplevel)/.mstack"
git log <base>..HEAD --grep="^WIP:" --format="%H%n%B%n---END---" > \
  "$(git rev-parse --show-toplevel)/.mstack/wip-context-before-squash.md" 2>/dev/null || true
```
...
> You are executing the /document-release workflow after a code push. Read the full skill file `${HOME}/.factory/skills/mstack/document-release/SKILL.md` and execute its complete workflow end-to-end, including CHANGELOG clobber protection, doc exclusions, risky-change gates, and named staging. Do NOT attempt to edit the PR body — no PR exists yet. Branch: `<branch>`, base: `<base>`.
>
...
**Always update the PR title to start with `v$NEW_VERSION`.** PR titles use the workspace-aware format `v<NEW_VERSION> <type>: <summary>` — version ALWAYS first, no exceptions, no "custom title kept intentionally" escape hatch. The shared helper `bin/mstack-pr-title-rewrite.sh` is the single source of truth for the rule.

1. Read the current title: `CURRENT=$(gh pr view --json title -q .title)` (or `glab mr view -F json | jq -r .title`).
2. Compute the corrected title: `NEW_TITLE=$($MSTACK_ROOT/bin/mstack-pr-title-rewrite.sh "$NEW_VERSION" "$CURRENT")`. The helper handles three cases: title already correct (no-op), title has a different `v<X.Y.Z.W>` prefix (replace it), or title has no version prefix (prepend one).
...
# (See Step 19 idempotency block + bin/mstack-pr-title-rewrite.sh for the rule.)
...
Append to `~/.mstack/projects/$SLUG/$BRANCH-reviews.jsonl`:

```bash
echo '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"BRANCH"}' >> ~/.mstack/projects/$SLUG/$BRANCH-reviews.jsonl
```
