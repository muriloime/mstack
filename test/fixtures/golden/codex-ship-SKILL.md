...
MSTACK_ROOT="$HOME/.codex/skills/mstack"
[ -n "$_ROOT" ] && [ -d "$_ROOT/.agents/skills/mstack" ] && MSTACK_ROOT="$_ROOT/.agents/skills/mstack"
MSTACK_BIN="$MSTACK_ROOT/bin"
MSTACK_BROWSE="$MSTACK_ROOT/browse/dist"
MSTACK_DESIGN="$MSTACK_ROOT/design/dist"
_UPD=$($MSTACK_BIN/mstack-update-check 2>/dev/null || .agents/skills/mstack/bin/mstack-update-check 2>/dev/null || true)
...
_VENDORED="no"
if [ -d ".agents/skills/mstack" ] && [ ! -L ".agents/skills/mstack" ]; then
  if [ -f ".agents/skills/mstack/VERSION" ] || [ -d ".agents/skills/mstack/.git" ]; then
    _VENDORED="yes"
  fi
fi
...
> This project has mstack vendored in `.agents/skills/mstack/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r .agents/skills/mstack/`
2. Run `echo '.agents/skills/mstack/' >> .gitignore`
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `.agents/skills/mstack/review/TODOS-format.md`).
...
1. Read `.agents/skills/mstack/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
2. **Read `.agents/skills/mstack/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
> You are classifying Greptile review comments for a /ship workflow. Read `.agents/skills/mstack/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>
...
Read `.agents/skills/mstack/review/TODOS-format.md` for the canonical format reference.
...
> You are executing the /document-release workflow after a code push. Read the full skill file `${HOME}/.agents/skills/mstack/document-release/SKILL.md` and execute its complete workflow end-to-end, including CHANGELOG clobber protection, doc exclusions, risky-change gates, and named staging. Do NOT attempt to edit the PR body — no PR exists yet. Branch: `<branch>`, base: `<base>`.
>
