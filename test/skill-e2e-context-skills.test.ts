...
function setupWorkdir(suffix: string): { workDir: string; mstackHome: string; slug: string } {
  const workDir = fs.mkdtempSync(path.join(os.tmpdir(), `skill-e2e-ctx-${suffix}-`));
  const mstackHome = path.join(workDir, '.mstack-home');
  ...
  for (const script of [
    'mstack-timeline-log', 'mstack-timeline-read', 'mstack-slug',
    'mstack-learnings-log', 'mstack-learnings-search',
    'mstack-update-check', 'mstack-config', 'mstack-repo-mode',
  ]) {
...
Environment:
- Use MSTACK_HOME="${mstackHome}" for all mstack bin scripts.
...
`);
  ...
  return { workDir, mstackHome, slug };
}

// Helper: seed a saved-context file into the storage dir.
function seedSave(mstackHome: string, slug: string, filename: string, frontmatter: Record<string, string>, body: string) {
  const dir = path.join(mstackHome, 'projects', slug, 'checkpoints');
...
describeIfSelected('Context Skills E2E (live-fire)', [
  ...
  testConcurrentIfSelected('context-save-routing', async () => {
    const { workDir, mstackHome, slug } = setupWorkdir('routing');

    // Prompt pattern: the slash command + explicit "invoke via Skill tool"
    // instruction. The MSTACK_HOME / ./bin bash setup that used to be in
    ...
    const checkpointDir = path.join(mstackHome, 'projects', slug, 'checkpoints');
  ...
  testConcurrentIfSelected('context-save-then-restore-roundtrip', async () => {
    const { workDir, mstackHome, slug } = setupWorkdir('roundtrip');
...
Do NOT use AskUserQuestion.`,
  ...
  testConcurrentIfSelected('context-restore-fragment-match', async () => {
    const { workDir, mstackHome, slug } = setupWorkdir('fragment');

    // Seed three saves with distinct titles.
    seedSave(mstackHome, slug, '20260101-120000-alpha-feature.md',
    ...
    seedSave(mstackHome, slug, '20260202-120000-middle-payments.md',
    ...
    seedSave(mstackHome, slug, '20260303-120000-omega-release.md',
  ...
  testConcurrentIfSelected('context-restore-empty-state', async () => {
    const { workDir, mstackHome, slug } = setupWorkdir('empty');
  ...
  testConcurrentIfSelected('context-restore-list-delegates', async () => {
    const { workDir, mstackHome, slug } = setupWorkdir('delegates');
    seedSave(mstackHome, slug, '20260101-120000-seed.md',
  ...
  testConcurrentIfSelected('context-restore-legacy-compat', async () => {
    const { workDir, mstackHome, slug } = setupWorkdir('legacy');
    ...
    // exactly this reason.
    seedSave(mstackHome, slug, '20260301-120000-legacy-pre-rename-work.md',
      {
        status: 'in-progress',
  ...
  testConcurrentIfSelected('context-save-list-current-branch', async () => {
    const { workDir, mstackHome, slug } = setupWorkdir('list-current');

    // Seed 3 files on 3 different branches. Current branch is "main".
    seedSave(mstackHome, slug, '20260101-120000-main-work.md',
      { status: 'in-progress', branch: 'main', timestamp: '2026-01-01T12:00:00Z' },
      '## Working on: main work LISTCURR_MAIN_TOKEN\n');
    seedSave(mstackHome, slug, '20260202-120000-feat-alpha.md',
      { status: 'in-progress', branch: 'feat/alpha', timestamp: '2026-02-02T12:00:00Z' },
      '## Working on: alpha LISTCURR_ALPHA_TOKEN\n');
    seedSave(mstackHome, slug, '20260303-120000-feat-beta.md',
  ...
  testConcurrentIfSelected('context-save-list-all-branches', async () => {
    const { workDir, mstackHome, slug } = setupWorkdir('list-all');
