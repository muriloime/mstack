 ...
 * `~/.mstack-brain-remote.txt` references survive the v1.27.0.0 rename.
 *
 ...
 *
 * Allowlist: the migration script (`mstack-upgrade/migrations/v1.27.0.0.sh`)
...
const ALLOWLIST = [
  ...
  'mstack-upgrade/migrations/v1.27.0.0.sh',
  // Older migration scripts — historical references; these document past state.
  'mstack-upgrade/migrations/v1.17.0.0.sh',
  ...
  'test/mstack-upgrade-migration-v1_17_0_0.test.ts',
  ...
  // The bin/mstack-config comment explicitly preserves the rename note.
  'bin/mstack-config',
  // Detect script's "renamed in v1.27.0.0" comment + brain-remote-fallback path.
  'bin/mstack-gbrain-detect',
  ...
  'bin/mstack-gbrain-source-wireup',
  ...
  // mstack-upgrade.test.ts may exercise old migration behavior.
  'test/mstack-upgrade.test.ts',
...
describe('no stale mstack-brain refs (v1.27.0.0 rename)', () => {
