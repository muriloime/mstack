...
const scriptPath = join(import.meta.dir, "..", "bin", "mstack-global-discover.ts");

describe("mstack-global-discover", () => {
  describe("normalizeRemoteUrl", () => {
    // Dynamically import to test the exported function
    let normalizeRemoteUrl: (url: string) => string;

    beforeEach(async () => {
      const mod = await import("../bin/mstack-global-discover.ts");
  ...
  describe("codex large session_meta parsing", () => {
    ...
    beforeEach(() => {
      tmpDir = mkdtempSync(join(tmpdir(), "mstack-codex-test-"));
