...
describeIfSelected('Review Dashboard Via Attribution', ['review-dashboard-via'], () => {
  ...
  beforeAll(() => {
    ...
    // mstack-review-read reads from ~/.mstack/projects/$SLUG/$BRANCH-reviews.jsonl
    // For the test, we'll write a mock mstack-review-read script that returns our test data
    ...
    // Write a mock mstack-review-read that returns our test data
    const mockBinDir = path.join(dashDir, '.mock-bin');
    fs.mkdirSync(mockBinDir, { recursive: true });
    fs.writeFileSync(path.join(mockBinDir, 'mstack-review-read'), [
      '#!/usr/bin/env bash',
    ...
    fs.chmodSync(path.join(mockBinDir, 'mstack-review-read'), 0o755);
...
Instead of running ~/.claude/skills/mstack/bin/mstack-review-read, run this mock: ${mockBinDir}/mstack-review-read
