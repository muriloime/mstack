 ...
 * auto-deciding a specific question via `mstack-question-preference --write
 ...
 *
 * Set up:
 *   - tmpDir as MSTACK_HOME (isolated state, doesn't touch the user's
 *     real ~/.mstack)
...
describeE2E('AUTO_DECIDE opt-in preserved under Conductor flags (periodic)', () => {
  test('user-opted-in question still auto-decides when AskUserQuestion is --disallowedTools', async () => {
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'mstack-auto-decide-'));
    try {
      // 1. Bootstrap the tmp MSTACK_HOME with question_tuning=true.
      const configBin = path.join(ROOT, 'bin', 'mstack-config');
      const setRes = spawnSync(configBin, ['set', 'question_tuning', 'true'], {
        env: { ...process.env, MSTACK_HOME: tmpHome },
        encoding: 'utf-8',
      });
      if (setRes.status !== 0) {
        throw new Error(`mstack-config set failed: ${setRes.stderr || setRes.stdout}`);
      }
      ...
      const slugBin = path.join(ROOT, 'bin', 'mstack-slug');
      const slugRes = spawnSync(slugBin, [], {
        cwd: ROOT,
        env: { ...process.env, MSTACK_HOME: tmpHome },
        encoding: 'utf-8',
      });
      // mstack-slug emits `eval`-able shell exports like `SLUG=garrytan-gstack`.
      ...
      const prefBin = path.join(ROOT, 'bin', 'mstack-question-preference');
      ...
      if (writeRes.status !== 0) {
        throw new Error(`mstack-question-preference --write failed: ${writeRes.stderr || writeRes.stdout}`);
      }
