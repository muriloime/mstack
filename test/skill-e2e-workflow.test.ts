...
describeIfSelected('Ship workflow E2E', ['ship-local-workflow'], () => {
  ...
  beforeAll(() => {
    shipRemoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'mstack-ship-remote-'));
    shipWorkDir = fs.mkdtempSync(path.join(os.tmpdir(), 'mstack-ship-work-'));
...
describeIfSelected('mstack-upgrade E2E', ['mstack-upgrade-happy-path'], () => {
  ...
  testConcurrentIfSelected('mstack-upgrade-happy-path', async () => {
    const mockGstack = path.join(upgradeDir, '.claude', 'skills', 'mstack');
    const result = await runSkillTest({
...
IMPORTANT: The install directory is at ./.claude/skills/mstack — use that exact path.`,
    ...
    recordE2E(evalCollector, '/mstack-upgrade happy path', 'mstack-upgrade E2E', result, {
