...
Report the exact output — either "READY: <path>" or "NEEDS_SETUP".`,
  ...
  testConcurrentIfSelected('operational-learning', async () => {
    ...
    const mstackHome = path.join(opDir, '.mstack-home');
    ...
    for (const script of ['mstack-learnings-log', 'mstack-slug']) {
      fs.copyFileSync(path.join(ROOT, 'bin', script), path.join(binDir, script));
      fs.chmodSync(path.join(binDir, script), 0o755);
    }

    // mstack-learnings-log will create the project dir automatically via mstack-slug
...
\`\`\`bash
MSTACK_HOME="${mstackHome}" ${binDir}/mstack-learnings-log '{"skill":"qa","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
\`\`\`
...
Log the operational learning now. Then say what you logged.`,
    ...
    const projectsDir = path.join(mstackHome, 'projects');
