/**
 * mstack-config explain_level round-trip + validation tests.
 *
 * Coverage:
...
const BIN_CONFIG = path.join(ROOT, 'bin', 'mstack-config');

let tmpHome: string;

beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'mstack-cfg-test-'));
});
...
function run(...args: string[]): { stdout: string; stderr: string; status: number } {
  const res = spawnSync(BIN_CONFIG, args, {
    env: { ...process.env, MSTACK_STATE_DIR: tmpHome },
    encoding: 'utf-8',
    cwd: ROOT,
  });
  return {
...
describe('mstack-config explain_level', () => {
