...
1. **Always spawn, never redirect.** When user asks to use ANY mstack skill,
...
→ sessions_spawn(runtime: "acp", prompt: "<mstack-lite content>\n\n<task>")

**HEAVY:** needs a specific mstack methodology
→ sessions_spawn(runtime: "acp", prompt: "Load mstack. Run /qa https://...")
  Skills: /cso, /review, /qa, /ship, /investigate, /design-review, /benchmark, /mstack-upgrade

**FULL:** build a complete feature, multi-day scope, needs planning + review
→ sessions_spawn(runtime: "acp", prompt: "<mstack-full content>\n\n<task>")
...
→ sessions_spawn(runtime: "acp", prompt: "<mstack-plan content>\n\n<task>")
...
- "Upgrade mstack", "update mstack" → **HEAVY** with `Run /mstack-upgrade`
