...
```bash
~/.claude/skills/mstack/bin/mstack-question-log '{"skill":"document-generate","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
find . -type f -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.mstack/*" -not -path "./dist/*" -not -path "./build/*" -not -path "./.next/*" | head -200
```
