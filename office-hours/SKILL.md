...
description: |
  ...
  Use before /plan-ceo-review or /plan-eng-review. (mstack)
allowed-tools:
  - Bash
  - Read
  - Grep
...
gbrain:
  ...
  context_queries:
    ...
    - id: builder-profile
      kind: filesystem
      glob: "~/.mstack/builder-profile.jsonl"
      tail: 1
      render_as: "## Your builder profile snapshot"
    - id: design-doc-history
      kind: filesystem
      glob: "~/.mstack/projects/{repo_slug}/*-design-*.md"
      sort: mtime_desc
      limit: 3
      render_as: "## Recent design docs for this project"
    - id: prior-eureka
      kind: filesystem
      glob: "~/.mstack/analytics/eureka.jsonl"
      tail: 5
...
```bash
~/.claude/skills/mstack/bin/mstack-question-log '{"skill":"office-hours","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/mstack/browse/dist/browse" ] && B="$_ROOT/.claude/skills/mstack/browse/dist/browse"
[ -z "$B" ] && B="$HOME/.claude/skills/mstack/browse/dist/browse"
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
If `NEEDS_SETUP`:
1. Tell the user: "mstack browse needs a one-time build (~10 seconds). OK to proceed?" Then STOP and wait.
...
4. **List existing design docs for this project:**
   ...
   ls -t ~/.mstack/projects/$SLUG/*-design-*.md 2>/dev/null
   ```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
grep -li "<keyword1>\|<keyword2>\|<keyword3>" ~/.mstack/projects/$SLUG/*-design-*.md 2>/dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `~/.mstack/projects/`.
...
```bash
CODEX_PROMPT_FILE=$(mktemp /tmp/mstack-codex-oh-XXXXXXXX.txt)
```
...
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
D=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/mstack/design/dist/design" ] && D="$_ROOT/.claude/skills/mstack/design/dist/design"
[ -z "$D" ] && D="$HOME/.claude/skills/mstack/design/dist/design"
...
```bash
eval "$(~/.claude/skills/mstack/bin/mstack-slug 2>/dev/null)"
_DESIGN_DIR="$HOME/.mstack/projects/$SLUG/designs/mockup-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
...
```bash
SKETCH_FILE="/tmp/mstack-sketch-$(date +%s).html"
```
...
$B screenshot /tmp/mstack-sketch.png
```
...
The screenshot file at `/tmp/mstack-sketch.png` can be referenced by downstream skills
...
```bash
eval "$(~/.claude/skills/mstack/bin/mstack-paths)"
echo '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' >> "$MSTACK_STATE_ROOT/builder-profile.jsonl"
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
PRIOR=$(ls -t ~/.mstack/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `~/.mstack/projects/{slug}/{user}-{branch}-design-{datetime}.md`.
...
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.mstack/analytics/spec-review.jsonl 2>/dev/null || true
```
...
```bash
PROFILE=$(~/.claude/skills/mstack/bin/mstack-builder-profile 2>/dev/null) || PROFILE="SESSION_COUNT: 0
TIER: introduction"
...
- If yes: run `open https://ycombinator.com/apply?ref=mstack` and say: "Bring this design doc to your YC interview. It's better than most pitch decks."
...
>
> **ycombinator.com/apply?ref=mstack**
...
**Builder Journey Summary** (session 5+): Auto-generate `~/.mstack/builder-journey.md`
...
```bash
eval "$(~/.claude/skills/mstack/bin/mstack-paths)"
open "$MSTACK_STATE_ROOT/builder-journey.md"
```
...
Auto-generate updated `~/.mstack/builder-journey.md` with narrative arc. Open it.
...
```bash
eval "$(~/.claude/skills/mstack/bin/mstack-paths)"
echo '{"date":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","mode":"resources","project_slug":"'"${SLUG:-unknown}"'","signal_count":0,"signals":[],"design_doc":"","assignment":"","resources_shown":["URL1","URL2","URL3"],"topics":[]}' >> "$MSTACK_STATE_ROOT/builder-profile.jsonl"
```
...
The design doc at `~/.mstack/projects/` is automatically discoverable by downstream skills — they will read it during their pre-review system audit.
