...
description: |
  ...
  should be reviewed before implementation. (mstack)
allowed-tools:
  - Read
  - Edit
  - Grep
...
```bash
~/.claude/skills/mstack/bin/mstack-question-log '{"skill":"plan-design-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
### The mstack designer — YOUR PRIMARY TOOL

You have the **mstack designer**, an AI mockup generator that creates real visual mockups
...
MUST be saved to `~/.mstack/projects/$SLUG/designs/`, NEVER to `.context/`,
...
mstack designer is available (`DESIGN_READY` was printed during setup), **generate
mockups immediately.** Do not ask permission. This is the default behavior.

Tell the user: "Generating visual mockups with the mstack designer. This is how we
...
`~/.mstack/projects/$SLUG/designs/` (user config directory, not project files).
Mockups are design artifacts that inform the plan, not code changes. The mstack
...
- `mkdir -p ~/.mstack/projects/$SLUG/designs/...`
...
```bash
eval "$(~/.claude/skills/mstack/bin/mstack-slug 2>/dev/null)"
_DESIGN_DIR="$HOME/.mstack/projects/$SLUG/designs/<screen-name>-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
...
**If `DESIGN_NOT_AVAILABLE`:** Tell the user: "The mstack designer isn't set up yet. Run `$D setup` to enable visual mockups. Proceeding with text-only review, but you're missing the best part." Then proceed to review passes with text-based review.
...
**Log the result:**
```bash
~/.claude/skills/mstack/bin/mstack-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
$D generate --brief "<description of what 10/10 looks like for this dimension>" --output /tmp/mstack-ideal-<dimension>.png
```
...
Source: [OpenAI "Designing Delightful Frontends with GPT-5.4"](https://developers.openai.com/blog/designing-delightful-frontends-with-gpt-5-4) (Mar 2026) + mstack design methodology.
...
|----------------|-------------|-----------|-------|
| [screen name]  | ~/.mstack/projects/$SLUG/designs/[folder]/[filename].png | [brief description] | [constraints from review] |
```
...
`~/.mstack/` (user config directory, not project files). The skill preamble
already writes to `~/.mstack/sessions/` and `~/.mstack/analytics/` — this is
the same pattern. The review dashboard depends on this data. Skipping this
command breaks the review readiness dashboard in /ship.

```bash
~/.claude/skills/mstack/bin/mstack-review-log '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
...
4. If a plan file is in context for this skill invocation: confirm
   `mstack-review-log` was called and `mstack-review-read` was run at least
