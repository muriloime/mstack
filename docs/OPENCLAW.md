# mstack x OpenClaw Integration

mstack integrates with OpenClaw as a methodology source, not a ported codebase.
OpenClaw's ACP runtime spawns Claude Code sessions natively. mstack provides the
...
```
  ...
  calendar, memory, EA                     methodology + planning
       ...
       │   investigate, retro                   │
       │                                        ├── Generates mstack-lite
       ├── sessions_spawn(runtime: "acp")       │   (planning discipline)
       │       │                                │
       │       └── Claude Code                  ├── Generates mstack-full
       │           └── mstack installed at      │   (complete pipeline)
...
OpenClaw decides at spawn time which tier of mstack support to use:

| Tier | When | Prompt prefix |
|------|------|---------------|
| **Simple** | One-file edits, typos, config changes | No mstack context injected |
| **Medium** | Multi-file features, refactors | mstack-lite CLAUDE.md appended |
| **Heavy** | Specific mstack skill needed | "Load mstack. Run /X" |
| **Full** | Complete features, objectives, projects | mstack-full pipeline appended |
| **Plan** | "Help me plan a Claude Code project" | mstack-plan pipeline appended |
...
The complete ready-to-paste section lives in `openclaw/agents-mstack-section.md`.
Copy it into your OpenClaw AGENTS.md.

Key behavioral rules (these go ABOVE the dispatch tiers):

1. **Always spawn, never redirect.** When the user asks to use ANY mstack skill,
...
mstack-lite/full as a new section. Do not replace the repo's existing instructions.

## What mstack generates for OpenClaw

All artifacts live in the `openclaw/` directory and are generated by
`bun run gen:skill-docs --host openclaw`:

### mstack-lite (Medium tier)
`openclaw/mstack-lite-CLAUDE.md` — ~15 lines of planning discipline:
...
### mstack-full (Full tier)
`openclaw/mstack-full-CLAUDE.md` — chains existing mstack skills:
...
### mstack-plan (Plan tier)
`openclaw/mstack-plan-CLAUDE.md` — full review gauntlet, no implementation:
...
- `mstack-openclaw-office-hours` — Product interrogation (6 forcing questions)
- `mstack-openclaw-ceo-review` — Strategic challenge (10-section review, 4 modes)
- `mstack-openclaw-investigate` — Operational debugging (4-phase methodology)
- `mstack-openclaw-retro` — Operational retrospective (weekly review)

Source lives in `openclaw/skills/` in the mstack repo. These are hand-crafted
adaptations of the mstack methodology for OpenClaw's conversational context.
No mstack infrastructure (no browse, no telemetry, no preamble).

## Spawned session detection

When Claude Code runs inside a session spawned by OpenClaw, the `OPENCLAW_SESSION`
environment variable should be set. mstack detects this and adjusts:
...
For OpenClaw users: tell your OpenClaw agent "install mstack for openclaw."

The agent should:
1. Install mstack-lite CLAUDE.md into its coding session templates
2. Install the 4 native methodology skills
3. Add dispatch routing to AGENTS.md
4. Verify with a test spawn

For mstack developers: `./setup --host openclaw` outputs this documentation.
...
- No SOUL.md from mstack (OpenClaw has its own)
