...
- **Path traversal guarded** on `/welcome` — `MSTACK_SLUG` must match `^[a-z0-9_-]+$` or falls back to the built-in template.
- **SSRF guards** on `goto`, `download`, and scrape paths — validates URL target against a localhost/private-range blocklist.
- **Tunnel surface denial logging.** Every rejection on the tunnel listener (`path_not_on_tunnel`, `root_token_on_tunnel`, `missing_scoped_token`, `disallowed_command:*`) is appended to `~/.mstack/security/attempts.jsonl` with timestamp, source IP, path, method. Rate-capped at 60 writes/min.
...
```bash
$B pair-agent --local openclaw    # writes to ~/.openclaw/skills/mstack/browse-remote.json
$B pair-agent --local codex       # writes to ~/.codex/skills/mstack/browse-remote.json
$B pair-agent --local cursor      # writes to ~/.cursor/skills/mstack/browse-remote.json
```
...
3. Save it: `echo 'NGROK_AUTHTOKEN=your_token' > ~/.mstack/ngrok.env`
4. Optionally claim a stable domain: `echo 'NGROK_DOMAIN=your-name.ngrok-free.dev' >> ~/.mstack/ngrok.env`
