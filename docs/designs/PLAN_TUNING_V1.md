...
This plan exists because of **[Louise de Sadeleer](https://x.com/LouiseDSadeleer/status/2045139351227478199)**, who sat through a complete mstack run as a non-technical user and told us the truth about how it feels. Her specific feedback:
...
3. **Terse opt-out** (`mstack-config set explain_level terse`). Binary: `default` vs `terse`. Terse skips the Writing Style block entirely and uses V0 prose style.
4. **Host-aware preamble echo.** `_EXPLAIN_LEVEL=$(${binDir}/mstack-config get explain_level 2>/dev/null || echo "default")`. Host-portable via existing V0 `ctx.paths.binDir` pattern.
5. **mstack-config validation.** Document `explain_level: default|terse` in header. Whitelist values. Warn on unknown with specific message + default to `default`.
...
11. **Upgrade migration** (`mstack-upgrade/migrations/v<VERSION>.sh`). One-time post-upgrade interactive prompt offering to restore V0 prose via `explain_level: terse` for users who prefer it. Flag-file gated.
...
- **User-editable jargon list at runtime.** Revision 3 proposed `~/.mstack/jargon-list.json` as user override. Codex Pass 2 caught the contradiction with gen-time inlining. Resolved: repo-owned only, PRs to add/remove, regenerate to take effect.
...
~/.claude/skills/mstack/bin/
  mstack-config                     # MODIFIED: explain_level header + validation

mstack-upgrade/migrations/
...
Preamble bash (per-invocation):
  _EXPLAIN_LEVEL=$(${binDir}/mstack-config get explain_level 2>/dev/null || "default")
...
**User-editable at runtime (rejected):** `~/.mstack/jargon-list.json` overrides `scripts/jargon-list.json`.
...
Review report persisted in `~/.mstack/` via `mstack-review-log`. Plan file retained with full history at `~/.claude/plans/system-instruction-you-are-working-transient-sunbeam.md`.
