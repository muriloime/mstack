...
mstack projects... CLAUDE.md works as-is.
...
### Critical env vars for mstack integration
...
Without native `.slate/` publishing, mstack skills vanish when this flag is set.
...
mstack skills already work in Slate via the `.claude/skills/` fallback path.
No changes needed for basic functionality. Users who install mstack for Claude Code
...
- Host names duplicated in `bin/mstack-platform-detect`, `bin/mstack-uninstall`, `bin/dev-setup`
...
- `lib/worktree.ts` only copies `.agents/`, not `.slate/` — E2E tests in worktrees won't
  have Slate skills
- `bin/mstack-uninstall` doesn't know about `.slate/`
- `bin/dev-setup` doesn't wire `.slate/` for contributor dev mode
- `bin/mstack-platform-detect` doesn't detect Slate
