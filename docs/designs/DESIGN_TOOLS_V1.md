# Design: mstack Visual Design Generation (`design` binary)
...
**Screenshot consent (first-time only for $D evolve):** "This will send a screenshot of your live site to OpenAI for design evolution. [Proceed] [Don't ask again]" Stored in ~/.mstack/config.yaml as design_screenshot_consent.
...
- Exploration variants go to `/tmp/mstack-mockups-{session}/` (ephemeral, not committed)
...
- Fallback: if not in a git repo, save to `/tmp/mstack-mockup-{timestamp}.png`
...
```typescript
// claude host:
designDir: '~/.claude/skills/mstack/design/dist'
// codex host:
designDir: '$MSTACK_DESIGN'
```
Note: Codex runtime setup (`setup` script) must also export `MSTACK_DESIGN` env var, similar to how `MSTACK_BROWSE` is set.
...
[ -z "$D" ] && D=~/.claude/skills/mstack/design/dist/design
if [ -x "$D" ]; then
  echo "DESIGN_READY: $D"
else
...
| `scripts/resolvers/preamble.ts` | Add `MSTACK_DESIGN` env var export for Codex host |
...
1. Read `~/.mstack/openai.json` → `{ "api_key": "sk-..." }` (file permissions 0600)
...
3. If neither exists → guided setup flow:
   ...
   - Write to `~/.mstack/openai.json` with 0600 permissions
...
- Symlinked via existing `~/.claude/skills/mstack/` install path
...
- Auth module (read ~/.mstack/openai.json, fallback to env var, guided setup flow)
...
- Add MSTACK_DESIGN env var export to `scripts/resolvers/preamble.ts` (Codex host)
