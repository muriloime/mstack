...
Structured, typed, confidence-scored learnings that every mstack skill can read and
write. The goal: after 20 sessions on the same codebase, mstack knows every
...
YC founders building with AI. The people who run mstack on real codebases 20+ times
...
mstack has four distinct persistence layers. They share storage patterns
(JSONL in `~/.mstack/projects/$SLUG/`) but serve different purposes:
...
What shipped:
- Learnings persistence at `~/.mstack/projects/{slug}/learnings.jsonl`
...
- E1: Adaptive specialist gating, auto-skip specialists with 0-finding track record.
  Store per-project hit rates via mstack-learnings-log. User can force with --security etc.
...
- E5: Cross-review finding dedup, read mstack-review-read for prior review entries.
  Suppress findings matching a prior user-skipped finding.
- E7: Specialist performance tracking, log per-specialist metrics via mstack-review-log.
...
- Session timeline: every skill auto-logs start/complete events to
  `~/.mstack/projects/$SLUG/timeline.jsonl`. Local-only, never sent anywhere,
...
- Predictive skill suggestion: if your last 3 sessions follow a pattern
  (review, ship, review), mstack suggests what you probably want next.
...
- Timeline binaries: `bin/mstack-timeline-log` and `bin/mstack-timeline-read`.
