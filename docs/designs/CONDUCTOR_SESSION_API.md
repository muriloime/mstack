...
When Claude controls your real browser via CDP (mstack `$B connect`), you look at two
windows: **Conductor** (to see Claude's thinking) and **Chrome** (to see Claude's actions).

mstack's Chrome extension Side Panel shows browse activity — every command, result,
...
A "Session" tab in the mstack Chrome extension Side Panel that shows:
...
## What mstack builds (extension side)
