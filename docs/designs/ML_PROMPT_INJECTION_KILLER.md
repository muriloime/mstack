...
**Date:** 2026-03-28
**CEO Plan:** ~/.mstack/projects/garrytan-gstack/ceo-plans/2026-03-28-sidebar-prompt-injection-defense.md

## The Problem

The mstack Chrome extension sidebar gives Claude bash access to control the browser.
...
```typescript
// Public API -- any mstack component can call these
...
```json
// ~/.mstack/security/attempts.jsonl
{
...
```
AskUserQuestion:
  "mstack just blocked a prompt injection attempt from {domain}. These detections
   are rare and valuable for improving defenses for all mstack users. Can we
...
2. Cache to ~/.mstack/models/browsesafe-bench/ (not re-downloaded in CI)
...
- mstack becomes the only open source tool with native-speed prompt injection detection.
...
**Limitation:** macOS-only (Linux would need OpenBLAS FFI). But mstack already
...
- [ ] Implement `loadModel()` with download-on-first-use to ~/.mstack/models/
