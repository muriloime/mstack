...
T+1-3s    Extension loads, sidebar opens
            ...
            └── tryAutoConnect() polls until window.mstackServerPort + token are set
...
2. Extension calls `new WebSocket(url, ['mstack-pty.<token>'])`.
3. Agent reads `Sec-WebSocket-Protocol`, strips `mstack-pty.`, validates
...
| `mstack-pty.<...>` (Sec-WebSocket-Protocol) | Browser memory only; agent `validTokens` Set | `/ws` upgrade auth | 30 min, auto-revoked on WS close |
...
| Button | Behavior |
|--------|----------|
| 🧹 Cleanup | `window.mstackInjectToTerminal(prompt)` — pipes a "remove ads/banners" instruction into the live PTY. claude in the terminal sees it and acts. |
...
The Inspector's "Send to Code" button uses the same `mstackInjectToTerminal`
