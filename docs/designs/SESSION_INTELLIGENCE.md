...
mstack already produces valuable artifacts that survive on disk: CEO plans,
...
survive compaction. Because nobody else has mstack's artifact architecture.

## The Insight

mstack already writes structured artifacts to `~/.mstack/projects/$SLUG/`:
- CEO plans: `ceo-plans/`
...
the agent checks `~/.mstack/projects/$SLUG/` for recent plans, reviews, and
...
Plan: ~/.mstack/projects/$SLUG/checkpoints/latest.md"
...
- Not a replacement for Claude's built-in compaction (that handles session
  state; we handle mstack artifacts)
