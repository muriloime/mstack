 ...
 * There is no shared loader. The preamble provides: session preferences,
 * repo mode detection, model overlays, and skill routing.
 */
...
// GBrain cross-machine sync (runs at skill start)
...
export function generatePreamble(ctx: TemplateContext): string {
  ...
  const sections = [
    ...
    // Plan-mode-skill semantics stays near the top: after bash (so _BRANCH env
    // vars are live) and before all onboarding gates so models read the
    // authoritative "AskUserQuestion satisfies plan mode's end-of-turn" rule
    // before any other instruction. Renders for all skills (not
    // interactive-gated); the text applies universally.
