#!/usr/bin/env bun
/**
 * Generate mstack/llms.txt — a single discoverable index of every mstack
 ...
 *
 * Output: mstack/llms.txt at repo root.
 *
...
const OUTPUT = path.join(ROOT, "mstack", "llms.txt");
 ...
 * the mstack convention; we collapse those into a single paragraph.
 */
 ...
 * in a full mstack checkout; absent on minimal installs. Returns {} if the
...
export async function generateLlmsTxt(
  ...
  lines.push("");
  lines.push(
    "> mstack is Garry's Stack: AI coding skills + a fast headless browser binary + a design CLI. This file indexes every capability so agents can discover and invoke them without crawling individual SKILL.md files.",
  );
  lines.push("");
  ...
  void (async () => {
    ...
    if (dryRun) {
      ...
      if (existing !== result.content) {
        console.error(
          "[gen-llms-txt] OUT OF DATE — run `bun run gen:skill-docs` to regenerate mstack/llms.txt",
        );
        process.exit(1);
      }
