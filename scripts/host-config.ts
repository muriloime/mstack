...
export interface HostConfig {
  ...
  // --- Path Configuration ---
  /** Global install path relative to $HOME (e.g., '.config/opencode/skills/mstack'). */
  globalRoot: string;
  /** Project-local skill path relative to repo root (e.g., '.opencode/skills/mstack'). */
  localSkillRoot: string;
  /** Gitignored directory under repo root for generated docs (e.g., '.opencode'). */
  hostSubdir: string;
  /** Whether preamble generates $MSTACK_ROOT env vars (true for non-Claude hosts). */
  usesEnvVars: boolean;
  ...
  /** Optional repo-local sidecar config (e.g., Codex uses .agents/skills/mstack). */
  sidecar?: {
    /** Sidecar path relative to repo root (e.g., '.agents/skills/mstack'). */
    path: string;
  ...
  // --- Install Behavior ---
  install: {
    /** Whether mstack-config skill_prefix applies (Claude only). */
    prefixable: boolean;
