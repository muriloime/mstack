 ...
 *
 * Resolution order:
 * 1. ~/.mstack/openai.json → { "api_key": "sk-..." }
...
const CONFIG_PATH = path.join(process.env.HOME || "~", ".mstack", "openai.json");

export function resolveApiKey(): string | null {
  // 1. Check ~/.mstack/openai.json
  try {
...
/**
 * Save an API key to ~/.mstack/openai.json with 0600 permissions.
 */
...
export function requireApiKey(): string {
  ...
  if (!key) {
    ...
    console.error("  or save to ~/.mstack/openai.json: { \"api_key\": \"sk-...\" }");
