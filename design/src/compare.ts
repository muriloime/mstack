...
<script>
  ...
  function postFeedback(feedback) {
    if (!window.__MSTACK_SERVER_URL) return Promise.resolve(null);
    return fetch(window.__MSTACK_SERVER_URL + '/api/feedback', {
      method: 'POST',
  ...
  function startProgressPolling() {
    if (!window.__MSTACK_SERVER_URL) return;
    var pollCount = 0;
    ...
    var pollInterval = setInterval(function() {
      ...
      fetch(window.__MSTACK_SERVER_URL + '/api/progress')
  ...
  function submitRegenerate(detail) {
    ...
    postFeedback(feedback).then(function(result) {
      ...
      } else if (window.__MSTACK_SERVER_URL) {
